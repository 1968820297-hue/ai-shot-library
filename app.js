// Modified: custom Shotcraft interface and local preview UX; see NOTICE.txt.
...
const savedTheme = (() => {
  try {
    ...
    return ['system', 'light', 'dark'].includes(value) ? value : 'dark';
  } catch {
    return 'dark';
  }
})();
...
function mediaMarkup(style, cardIndex) {
  ...
  return `
    <figure class="preview">
      <video class="lazy-media" data-src="${escapeHtml(style.media.url)}" poster="./posters/${escapeHtml(style.media.url.split('/').pop().split('?')[0].replace(/\.[^.]+$/, '.jpg'))}" muted loop playsinline preload="none"
...
function cardMatches(card) {
  if (state.selectedOnly && !state.selectedCards.has(card.name)) return false;
  const searchable = [
    card.name,
...
function render() {
  ...
  document.querySelector('#resultCount').textContent = cards.length;
  document.querySelector('#viewCount').textContent = state.language === 'zh' ? `${cards.length} 个模板` : `${cards.length} templates`;
  const categoryLabel = elements.filters.querySelector(`[data-filter="${state.filter}"] [data-i18n]`)?.textContent || text('all');
  document.querySelector('#sectionTitle').textContent = state.selectedOnly ? text('navSelected') : categoryLabel;
  elements.emptyState.querySelector('h3').textContent = state.selectedOnly && !state.selectedCards.size ? (state.language === 'zh' ? '还没有选择镜头，先去模板库挑选吧。' : 'No shots selected yet. Explore the library to choose some.') : text('empty');
  ...
  mediaObserver = new IntersectionObserver((entries) => {
    entries.forEach((entry) => {
      ...
      if (entry.isIntersecting) {
        ...
        if (media.tagName === 'VIDEO' && !state.motionPaused && !document.querySelector('dialog[open]') && entry.intersectionRatio > 0.55) {
          media.play().catch(() => {});
        }
...
function updateSelectionBar() {
  ...
  document.querySelector('#navSelectedCount').textContent = count;
...
function openFullscreen(key) {
  ...
  if (!video) return;
  openPreview(video.dataset.src || video.currentSrc, video.getAttribute('aria-label'));
  return;
}
...
elements.library.addEventListener('click', (event) => {
  ...
  const recipeLink = event.target.closest('.source-link');
  if (recipeLink) {
    event.preventDefault();
    openRecipe(recipeLink.closest('.shot-card').id);
    return;
  }
  const preview = event.target.closest('.preview');
  if (preview) {
    const video = preview.querySelector('video');
    if (video) openFullscreen(video.dataset.key);
  }
...
function syncViewButtons() {
  document.querySelector('#allView').classList.toggle('is-active', !state.selectedOnly);
  document.querySelector('#selectedView').classList.toggle('is-active', state.selectedOnly);
  document.querySelector('#allView').setAttribute('aria-pressed', String(!state.selectedOnly));
  document.querySelector('#selectedView').setAttribute('aria-pressed', String(state.selectedOnly));
}

function chooseView(selectedOnly) {
  state.selectedOnly = selectedOnly;
  state.query = '';
  state.filter = 'all';
  clearTimeout(searchTimer);
  elements.searchInput.value = '';
...
document.querySelector('#allView').addEventListener('click', () => chooseView(false));
document.querySelector('#selectedView').addEventListener('click', () => chooseView(true));
...
guideDialog.querySelector('.dialog-close').addEventListener('click', () => guideDialog.close());
guideDialog.addEventListener('click', event => { if(event.target === guideDialog) {const r=guideDialog.getBoundingClientRect();if(event.clientX<r.left||event.clientX>r.right||event.clientY<r.top||event.clientY>r.bottom)guideDialog.close();} });

function openPreview(url, title) {
  const dialog = document.createElement('dialog');
  dialog.className = 'preview-dialog';
  dialog.setAttribute('aria-label', title || text('sample'));
  dialog.innerHTML = `<div class="dialog-heading"><span>${escapeHtml(title)}</span><button class="dialog-close" aria-label="${state.language === 'zh' ? '关闭预览' : 'Close preview'}">×</button></div><video controls autoplay playsinline muted loop src="${escapeHtml(url)}"></video><div class="preview-dialog-foot"><span>${state.language === 'zh' ? '使用播放器控制暂停、进度和全屏' : 'Use player controls to pause, scrub, and enter fullscreen'}</span><a href="${escapeHtml(url.split('?')[0])}" download>${state.language === 'zh' ? '下载样片 ↓' : 'Download preview ↓'}</a></div>`;
  document.body.append(dialog);
  document.querySelectorAll('video').forEach(v => {if(v !== dialog.querySelector('video')) v.pause()});
  dialog.querySelector('button').addEventListener('click', () => dialog.close());
  dialog.addEventListener('close', () => {dialog.querySelector('video').pause();dialog.remove();observeMedia();syncFeaturePlayback()});
...
document.querySelector('#featuredButton').addEventListener('click', () => {
  const card = state.library?.cards.find(c => c.name === 'carousel-3d');
  if(card)openPreview(card.styles[0].media.url, cardName(card));
});

async function openRecipe(name) {
  const card = state.library?.cards.find(c => c.name === name);
  if(!card)return;
  const dialog = document.createElement('dialog');
  dialog.className = 'guide-dialog recipe-dialog';
  dialog.setAttribute('aria-label', cardName(card));
  dialog.innerHTML = `<div class="dialog-heading"><span>${escapeHtml(card.name)}</span><button class="dialog-close" aria-label="${state.language === 'zh' ? '关闭配方' : 'Close recipe'}">×</button></div><h2>${escapeHtml(cardName(card))}</h2><pre class="recipe-content">${state.language === 'zh' ? '正在读取配方…' : 'Loading recipe…'}</pre><div class="guide-foot"><span>Video Shotcraft · Apache 2.0</span><a href="${escapeHtml(card.sourceUrl)}" download>${state.language === 'zh' ? '下载配方 ↓' : 'Download recipe ↓'}</a></div>`;
  document.body.append(dialog);
  dialog.querySelector('button').addEventListener('click', () => dialog.close());
  dialog.addEventListener('close', () => dialog.remove());
  dialog.showModal();
  try {const response=await fetch(card.sourceUrl);if(!response.ok)throw new Error('Recipe unavailable');dialog.querySelector('.recipe-content').textContent=await response.text();}
  catch {dialog.querySelector('.recipe-content').textContent=state.language === 'zh' ? '配方暂时无法读取，请关闭后重试。' : 'Could not load the recipe. Please close and retry.';}
}

const featureVideo = document.querySelector('#featureVideo');
let featureVisible = false;
function syncFeaturePlayback() {
  if (featureVisible && !state.motionPaused && !document.querySelector('dialog[open]')) featureVideo.play().catch(() => {});
...
function updateMotionButton() {
  ...
  label.dataset.i18n = state.motionPaused ? 'resumeMotion' : 'pauseMotion';
  label.textContent = text(label.dataset.i18n);
  button.querySelector('.motion-icon').textContent = state.motionPaused ? '▷' : 'Ⅱ';
}
function applyMotionPreference() {
  if (state.motionPaused) document.querySelectorAll('video:not(.preview-dialog video)').forEach(video => video.pause());
  updateMotionButton();
  observeMedia();
  syncFeaturePlayback();
}
...
new IntersectionObserver(entries => {
  featureVisible = entries[0].isIntersecting;
  syncFeaturePlayback();
}, {threshold:.1}).observe(featureVideo);

// The optional WebMCP interface uses the same filters as the visible library.
if(document.modelContext?.registerTool){
  const lifecycle = new AbortController();
  try { Promise.resolve(document.modelContext.registerTool({
    name:'filter_shot_library', title:'筛选镜头模板',
    description:'Filter the visible local shot library by keyword and an existing category; returns matching shot names. Does not download or send anything.',
    inputSchema:{type:'object',properties:{query:{type:'string'},category:{type:'string',enum:['all','opening','typography','ui-entrance','camera','data','interaction','transition','rhythm','effects','outro']}},required:['query','category'],additionalProperties:false},
    annotations:{readOnlyHint:false,untrustedContentHint:true},
    execute(input){
      if(!input||typeof input.query!=='string'||input.query.length>200||typeof input.category!=='string'||!elements.filters.querySelector(`[data-filter="${CSS.escape(input.category)}"]`))throw new Error('Invalid query or category');
      if(!state.library)throw new Error('Library is loading');
      chooseView(false);state.query=input.query.trim();state.filter=input.category;elements.searchInput.value=state.query;
      elements.filters.querySelectorAll('[data-filter]').forEach(b=>{b.classList.toggle('is-active',b.dataset.filter===state.filter);b.setAttribute('aria-pressed',String(b.dataset.filter===state.filter))});render();
      const cards=state.library.cards.filter(cardMatches);return {count:cards.length,shots:cards.map(c=>({name:c.name,title:cardName(c)}))};
    }
